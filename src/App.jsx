...
const apiBaseUrl = "https://condominio-api-pq4j.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Unidade</li>
          <li>✓ Controle completo de Morador</li>
          <li>✓ Controle completo de Cobranca</li>
          <li>✓ Controle completo de Visitante</li>
        </ul>
      </div>
...
function PainelUnidade({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["numero", "bloco", "tipo", "area", "vaga", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/unidades", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/unidades/" + editId : apiBaseUrl + "/api/unidades";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["numero", "bloco", "tipo", "area", "vaga"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/unidades/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
    ...
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["bloco"] ?? "Unidade")}</div>
            ...
            <div className="item-field"><b>vaga</b><span>{item["vaga"] != null ? (item["vaga"] ? "Sim" : "Não") : "—"}</span></div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Unidade</h3>
      ...
      <input type="text" value={form["bloco"] ?? ""} onChange={e => setForm({...form, bloco: e.target.value})} />
      ...
      <label className="field-label checkbox-label">
        <input type="checkbox" checked={form["vaga"] === true || form["vaga"] === "true"} onChange={e => setForm({...form, vaga: e.target.checked})} />
        vaga
      </label>
              <div className="modal-actions">
...
function PainelMorador({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [unidadeList, setUnidadeList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "unidadeId", "tipo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/moradors", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/unidades", { headers: authHeaders(token) }).then(r => r.json()).then(setUnidadeList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/moradors/" + editId : apiBaseUrl + "/api/moradors";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "unidadeId", "tipo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["unidade"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/moradors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Morador")}</div>
            ...
            <div className="item-field"><b>unidadeId</b><span>{item["unidadeId"] ?? "—"}</span></div>
            <div className="item-field"><b>tipo</b><span>{item["tipo"] ?? "—"}</span></div>
            <div className="item-field"><b>unidade</b><span>{item.unidadeId ? ((unidadeList.find(o => o.id === item.unidadeId) || {}).bloco ?? ("#" + item.unidadeId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Morador</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <label className="field-label">unidadeId</label>
      <input type="number" value={form["unidadeId"] ?? ""} onChange={e => setForm({...form, unidadeId: e.target.value})} />
      <label className="field-label">tipo</label>
      <input type="text" value={form["tipo"] ?? ""} onChange={e => setForm({...form, tipo: e.target.value})} />
      <label className="field-label">unidade</label>
      <select value={form["unidade"] ?? ""} onChange={e => setForm({...form, unidade: e.target.value})}>
        <option value="">Selecione...</option>
        {(unidadeList || []).map(o => (<option key={o.id} value={o.id}>{o.bloco ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelCobranca({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["unidadeId", "mesReferencia", "valorCondominio", "valorAgua", "valorGas", "valorTotal", "status", "dataVencimento", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/cobrancas", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/cobrancas/" + editId : apiBaseUrl + "/api/cobrancas";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["unidadeId", "mesReferencia", "valorCondominio", "valorAgua", "valorGas", "valorTotal", "status", "dataVencimento"].forEach(k => { f[k] = item[k] ?? ""; });
    ["unidade"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/cobrancas/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["unidadeId"] ?? "Cobranca")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/cobrancas/" + item.id, {
        ...
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            <div className="item-photo-wrap">
              ...
              <img className="item-photo" style={{display: "none"}} src={"https://picsum.photos/seed/cobranca" + item.id + "/320/200"} alt="Cobranca" loading="lazy" onLoad={e => { e.target.style.display = "block"; e.target.previousSibling.style.display = "none"; }} />
            </div>
            <div className="item-card-header">
              <div className="item-icon-badge">💳</div>
              <span className="item-id-tag">#{item.id}</span>
            </div>
            <div className="item-title">{String(item["unidadeId"] ?? "Cobranca")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>mesReferencia</b><span>{item["mesReferencia"] ?? "—"}</span></div>
            <div className="item-field"><b>valorCondominio</b><span>{item["valorCondominio"] != null ? Number(item["valorCondominio"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>valorAgua</b><span>{item["valorAgua"] != null ? Number(item["valorAgua"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            <div className="item-field"><b>valorGas</b><span>{item["valorGas"] != null ? Number(item["valorGas"]).toLocaleString("pt-BR", {style:"currency",currency:"BRL"}) : "—"}</span></div>
            ...
            <div className="item-field"><b>dataVencimento</b><span>{item["dataVencimento"] ? new Date(item["dataVencimento"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>unidade</b><span>{item.unidadeId ? ((unidadeList.find(o => o.id === item.unidadeId) || {}).bloco ?? ("#" + item.unidadeId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
              <button className="btn btn-ghost btn-sm" onClick={() => gerarPix(item.valorCondominio)}>Cobrar via Pix</button>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Cobranca</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">unidadeId</label>
      <input type="text" value={form["unidadeId"] ?? ""} onChange={e => setForm({...form, unidadeId: e.target.value})} />
      <label className="field-label">mesReferencia</label>
      <input type="text" value={form["mesReferencia"] ?? ""} onChange={e => setForm({...form, mesReferencia: e.target.value})} />
      <label className="field-label">valorCondominio</label>
      <input type="number" step="0.01" value={form["valorCondominio"] ?? ""} onChange={e => setForm({...form, valorCondominio: e.target.value})} />
      <label className="field-label">valorAgua</label>
      <input type="number" step="0.01" value={form["valorAgua"] ?? ""} onChange={e => setForm({...form, valorAgua: e.target.value})} />
      <label className="field-label">valorGas</label>
      <input type="number" step="0.01" value={form["valorGas"] ?? ""} onChange={e => setForm({...form, valorGas: e.target.value})} />
      ...
      <label className="field-label">dataVencimento</label>
      <input type="datetime-local" value={form["dataVencimento"] ?? ""} onChange={e => setForm({...form, dataVencimento: e.target.value})} />
      <label className="field-label">unidade</label>
      <select value={form["unidade"] ?? ""} onChange={e => setForm({...form, unidade: e.target.value})}>
        <option value="">Selecione...</option>
        {(unidadeList || []).map(o => (<option key={o.id} value={o.id}>{o.bloco ?? ("#" + o.id)}</option>))}
...
function PainelVisitante({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "unidadeId", "dataEntrada", "dataSaida", "motivo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/visitantes", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/visitantes/" + editId : apiBaseUrl + "/api/visitantes";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "unidadeId", "dataEntrada", "dataSaida", "motivo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["unidade"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/visitantes/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Visitante")}</div>
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Visitante</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">nome</label>
      <input type="text" value={form["nome"] ?? ""} onChange={e => setForm({...form, nome: e.target.value})} />
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Unidade");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Condominio<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Unidade" ? " active" : "")} onClick={() => setAba("Unidade")}><span className="nav-ico">🏢</span>Unidade</button>
        <button className={"nav-btn" + (aba === "Morador" ? " active" : "")} onClick={() => setAba("Morador")}><span className="nav-ico">🗂️</span>Morador</button>
        <button className={"nav-btn" + (aba === "Cobranca" ? " active" : "")} onClick={() => setAba("Cobranca")}><span className="nav-ico">💳</span>Cobranca</button>
        <button className={"nav-btn" + (aba === "Visitante" ? " active" : "")} onClick={() => setAba("Visitante")}><span className="nav-ico">📅</span>Visitante</button>
      ...
      <main className="main">
        ...
        {aba === "Unidade" && <PainelUnidade token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Morador" && <PainelMorador token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Cobranca" && <PainelCobranca token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Visitante" && <PainelVisitante token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      </main>
